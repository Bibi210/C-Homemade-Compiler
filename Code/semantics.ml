...
(*All gotos cause we need to verify them after the function definition*)
let gotos = ref []
...
(*Try to get ride of Nested Custom types*)
let outside_of_custom_type = function
  | Custom_t old_type -> old_type
  | x -> x
;;

(*Search through the type links to test types equality*)
(*Works somehow*)
let type_equal a b pos =
  let rec type_equal_aux base_a a_aux a_aux_pos other is_casted =
    ...
    | true ->
      ...
      then
        emit_warning
          (Printf.sprintf "Cast from %s to %s" (fmt_type base_a) (fmt_type other))
          a_aux_pos
      else ();
      true
    | false ->
...
(*Verify if a variable is assigned*)
        ...
        let args =
          List.map2
            (fun a b ->
              ...
              if type_equal analyse_result.base_type b called_func.pos
              then (
  ...
  | Syntax.Assign assign ->
    ...
    | Some (Var_t (v_type, _)) ->
      ...
      let analyze_var =
        ...
        | Lderef x -> analyze_expr (Syntax.Deref { var_name = x; pos = assign.pos }) env
      in
  ...
  | Syntax.Deref x ->
    ...
    | Some (Var_t (var_t, assigned)) ->
      ...
      | Pointer_t t ->
        ...
        else raise (Error (Printf.sprintf "Deref of never initialized Pointer", x.pos))
      | t ->
        Printf.printf "%s\n" (fmt_type t);
...
let rec analyze_instr intr env is_loop (*Is loop is there for Continue and Break Verifys*)
  =
  match intr with
  | Syntax.Expr x ->
  ...
  | Syntax.Return syntax_return ->
    ...
    | Func_t (ret_type, _) ->
      if type_equal ret_type returned.base_type syntax_return.pos
...
let analyze_def def env =
  ...
  | Syntax.Typedef syntax_typedef ->
    ...
    | Some _ -> raise (Error ("This type is already defined", syntax_typedef.pos))
    | None ->
      Hashtbl.add type_links_env syntax_typedef.new_type (Derived syntax_typedef.old_type);
      Hashtbl.add
        type_links_env
        (Pointer_t syntax_typedef.new_type)
...
let analyze parsed =
  ...
  if Option.is_some (Env.find_opt "main" env)
  then analysed
  else
    raise
      (Error
