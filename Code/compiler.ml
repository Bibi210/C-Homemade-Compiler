...
let fmt_lval = function Lvar x -> "" ^ x | Lderef x -> "Derf Pointer " ^ x
...
let fmt_instr begin_or_end instr c_info =
  let tmp =
    ...
    | Decl var ->
        Printf.sprintf "(Implicit) Stack Space Reserved for Var (%s) at -FP(%d)"
          var c_info.fpo
    | While (cond, _, do_mode) ->
        let com =
          "While "
          ^ string_of_int c_info.gen_label_counter
          ^ " condition : (" ^ fmt_expr cond c_info.env ^ ") is true"
        in
    ...
    | For (cond, incre, _) ->
        let com =
          "For "
          ^ string_of_int c_info.gen_label_counter
          ^ " condition : (" ^ fmt_expr cond c_info.env ^ ") "
          ^ "\n# Increment :(" ^ fmt_expr incre c_info.env ^ ")"
        in
        com
  ...
  in
  if String.equal "None" tmp then "None"
  else if begin_or_end == 0 then "Start of Instr : " ^ tmp
...
let rec compile_expr e env =
  ...
  | Assign (var, expr) -> (
      ...
      | Lderef pt_var ->
          [ Lw (Reg T0, Env.find pt_var env); Sw (V0, Mem (T0, 0)) ])
...
let rec compile_instr instr c_info =
  ...
  | For (cond, incre, block) ->
      ...
      in
      {
        ...
        code =
          c_info.code @ [ Jump_Lbl for_uniq ]
          @ compile_expr cond c_info.env
          @ [ Beq (V0, ZERO, "end_" ^ for_uniq) ]
          @ compiled_block_info.code @ [ Jump_Lbl for_incre ]
  ...
  | While (cond, block, do_mode) ->
        ...
        compile_instr block
          {
            ...
            code = [];
            top_loop_start =
              (if do_mode then "Test_" ^ while_uniq else while_uniq);
      ...
      in
      {
        ...
        code =
          (if do_mode then
           c_info.code @ [ Jump_Lbl while_uniq ] @ compiled_block_info.code
           @ compile_expr cond c_info.env
           ...
           @ [
            ...
            c_info.code @ [ Jump_Lbl while_uniq ]
            @ compile_expr cond c_info.env
            @ [ Beq (V0, ZERO, "end_" ^ while_uniq) ]
  ...
  | If (cond, block_true, block_false) ->
      ...
      in
      {
        ...
        code =
          ...
          @ true_block_info.code @ [ J end_uniq ] @ [ Jump_Lbl else_uniq ]
          @ false_block_info.code @ [ Jump_Lbl end_uniq ];
        fpo =
          c_info.fpo
  ...
  | NestedBlock block ->
      let compiled_block_info =
        compile_block block
          {
            c_info with
            gen_label_counter = c_info.gen_label_counter;
            code = [];
          }
      in
      {
  ...
  | Return expr ->
      {
        c_info with
        code = c_info.code @ compile_expr expr c_info.env @ [ Jr RA ];
      }
  | None -> c_info
...
let compile_def func count =
  ...
  | Func (name, args, block) ->
      ...
      ( [ Jump_Lbl name; Addi (SP, SP, -compiled_block_info.fpo) ]
        @ compiled_block_info.code @ [ Jr RA ],
  ...
  let code, _ = compile_prog ir 0 in
  {
    text =
      cleanup_asm
        ...
        @ [ Jr RA ] @ Baselib.builtins);
    data =
      List.of_seq (Seq.map (fun (str, label) -> (label, Asciiz str)) simplify);
  }
