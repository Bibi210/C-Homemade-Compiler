...
let _counter = ref 0 (*Help us generation string uniq IDS*)

let _clear_on = ref false (*If this is True then we cleanup code*)
...
and simplify_block block env =
  ...
  _clear_on := false;
  (*Reset the cleanup mode to false*)
  tmp
;;
